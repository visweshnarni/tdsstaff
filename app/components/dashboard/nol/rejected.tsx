...
export default function RejectedNOCList({ data }: Props) {
  const [search, setSearch] = useState("");
  const [currentPage, setCurrentPage] = useState(1);

  const filteredData = useMemo(() => {
    return data.filter((item) => {
      // Adjusted filter logic to search across all specified fields
      const values = [
      ...
      <Table className="w-full table-auto text-sm">
        <TableHeader>
          <TableRow>
            ...
            <TableHead className="text-center px-2 py-2">Membership Number</TableHead>
          ...
          {paginatedData.length > 0 ? (
            paginatedData.map((item) => (
              <TableRow key={item.applicationNumber}>
                <TableCell className="text-center px-2 py-2">{item.applicationNumber}</TableCell>
                <TableCell className="text-center px-2 py-2">{item.membershipNumber}</TableCell>
